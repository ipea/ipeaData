...
#' can be used for search for SERCODIGO of series. SERCODIGO
#'
...
#' Return metadata from a serie on Ipeadata .
#'
...
#' @export
#'
search_serie <- function(term = NULL, fields_search=c('SERCODIGO', 'SERNOME', 'PERNOME', 'UNINOME'), type='data.table'){
  ...
  query_search <- paste(begin_query, query_search, end_query, sep = "")
  api_call <- paste("http://ipeadata2-homologa.ipea.gov.br/api/v1/Metadados?", query_search, sep = "" )
  data <- basic_call(api_call, type)
  return(data)
